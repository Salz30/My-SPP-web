        ...
        <div class="card p-4">
            <form action="{{ route('siswa.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control form-control-lg bg-light border-0 @error('nisn') is-invalid @enderror" name="nisn" value="{{ old('nisn') }}" required placeholder="Masukkan angka NISN">
                    @error('nisn') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                
                <div class="mb-3">
                    <label class="form-label fw-bold text-dark">Nama Lengkap Siswa <span class="text-danger">*</span></label>
                    <input type="text" class="form-control form-control-lg bg-light border-0 @error('nama_siswa') is-invalid @enderror" name="nama_siswa" value="{{ old('nama_siswa') }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control form-control-lg bg-light border-0 @error('no_hp') is-invalid @enderror" name="no_hp" value="{{ old('no_hp') }}" placeholder="Contoh: 081234567890">
                ...
                <div class="mb-4">
                    ...
                    <select class="form-select form-select-lg bg-light border-0 @error('id_kelas') is-invalid @enderror" name="id_kelas" required>
                        ...
                        @foreach($kelas as $k)
                            <option value="{{ $k->id_kelas }}" {{ old('id_kelas') == $k->id_kelas ? 'selected' : '' }}>{{ $k->nama_kelas }} (Wali: {{ $k->wali_kelas }})</option>
                        @endforeach
                    </select>
                ...
                <div class="text-end">
                    <button type="submit" class="btn btn-primary px-5 shadow-sm">Simpan Siswa</button>
                </div>
            </form>
        </div>
    </div>
